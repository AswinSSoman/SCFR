...
def parse_header(header):
    ...
    2. SCFR headers (updated regex to handle lcl| prefixes and suffixes):
        >-2::NC_060925.1:2194196-2200379
        >lcl|ORF1_-2::NC_071990.2:23739917-23746157(-):33:6239 ...
    """

    # ===============================
    # Case 1 → SCFR HEADER (Updated Regex)
    # ===============================
    # The regex now looks for the SCFR pattern anywhere in the string,
    # capturing the frame, accession, and coordinates.
    # It accounts for the optional 'lcl|ORF*_', the strand '(-)', and the
    # ':start:end' suffix.
    scfr = re.search(r'([+-]?\d+)::([A-Za-z0-9_.]+):(\d+)-(\d+)', header)
    ...
    if scfr:
        ...
        # Attempt to extract the ORF identifier (e.g., ORF1, ORF2) from the start
        orf_match = re.search(r'(ORF\d+)_', header)
        orf_id = orf_match.group(1) if orf_match else "SCFR"

        return {
            "gene": f"{orf_id}_{acc}_{start}_{end}",
            "gene_id": orf_id,
            "prot_id": f"frame{frame}",
            "prot_name": header.split(':', 4)[-1].strip(),  # Use the rest of the line as protein name
            "acc": acc
        }
