...
if (length(args) < 2) {
  stop("Usage: Rscript plot_upstream_exon_feature_heatmap.R <input.tsv> <BIN_WIDTH> <MAX_LEN> <output.pdf>")
}
...
df <- df %>%
  filter(
    upstream_len_in_scfr >= -2,
    upstream_len_in_scfr <= MAX_LEN
  )

# ============================
# Create upstream bins
# ============================
...
df <- df %>%
  mutate(
    upstream_bin = cut(
      upstream_len_in_scfr,
      breaks = bin_breaks,
      include.lowest = TRUE,
      right = FALSE,
      labels = bin_labels
    ),
    upstream_bin = factor(upstream_bin, levels = bin_labels)
  )
...
df <- df %>%
  mutate(
    ...
    exon_order = first_exon_order,
    splicing = first_exon_splicing
  )
...
heatmap_df <- df %>%
  select(upstream_bin, exon_count_class, exon_order, splicing) %>%
  pivot_longer(
    cols = c(exon_count_class, exon_order, splicing),
    names_to = "group",
    values_to = "category"
  ) %>%
  count(upstream_bin, category) %>%
  complete(
    upstream_bin,
    category = factor(
      category,
...
bin_stats <- df %>%
  count(upstream_bin, name = "bin_count") %>%
  mutate(
...
p_heatmap <- ggplot(heatmap_df, aes(x = category, y = upstream_bin, fill = n)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(name = "Count") +
  labs(x = NULL, y = "Upstream length bins (bp)") +
...
p_counts <- ggplot(bin_stats, aes(y = upstream_bin, x = 0, label = label)) +
