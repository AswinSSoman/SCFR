...
if (length(args) != 8) {
  stop("Usage: Rscript plot_pca_codingstatus_biplot.R <input_folder> <output_folder> <coding_status.tsv> <species_name> <plot_width> <plot_height> <point_size> <length>")
}
...
for (f in c(scores_file, cluster_file, loadings_file, variance_file, coding_file)) {
...
coding   <- read_tsv(coding_file, show_col_types = FALSE)
...
plot_df <- scores %>%
  ...
  left_join(coding, by = c("sequence" = "SCFR")) %>%
  mutate(
    coding_label = case_when(
      coding_status == "in_frame_coding"  ~ "In-frame Coding SCFR",
      coding_status == "out_frame_coding" ~ "Out-of-frame Coding SCFR",
      coding_status == "non_coding"       ~ "Non-Coding SCFR",
      TRUE                                ~ "Non-Coding SCFR"   # fallback safety
    )
  )
...
for (pair in pc_pairs) {
  ...
  pdf_file <- file.path(
    output_folder,
    paste0("pca_codingstatus_", x_pc, "_", y_pc, ".pdf")
  )
...
theme(
  ...
  legend.key.width  = unit(2, "cm"),   # ← increases spacing between items
...
cat("All PCA plots saved colored by coding status categories.\n")
