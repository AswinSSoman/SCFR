...
# Median (lower half)
  ...
  geom_text(aes(
    y = min -150, # Adjusted Y offset
    label = paste0(min_kb)
  ), size=3.5) +
  
  # Max label (bp now, not kb)
  geom_text(aes(
    y = max +150, # Adjusted Y offset
    label = paste0(max_kb)
  ), size=3.5) +
  ...
  coord_cartesian(xlim = c(0.5, num_categories + 0.6),
    ylim = c(min(df$min) - 600, max(df$max) + 600), # Assuming min(df$min) is 1 and max(df$max) is 6801
    expand = FALSE
  ) +
