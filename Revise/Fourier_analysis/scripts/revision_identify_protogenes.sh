######################################################################################################################################################################################################################################################################################################
#																																																																																																																																				Identify Proto-genes from Intergenic SCFRs
######################################################################################################################################################################################################################################################################################################


######################################################################################################################################################################################################################################################################################################
#1. Filter Intergenic SCFRs (not antisense intergenic)

	#Check overlap between intergenic SCFR & regions with homology
	cd /media/aswin/SCFR/SCFR-main/Fourier_analysis/human/300
	bedtools intersect -a human_scfr_all_atleast_300bp_only_intergenic_unique.bed -b Non_homology/all_human_homology_regions.bed | wc -l
	bedtools intersect -a human_scfr_all_atleast_300bp_antisense_intergenic.bed -b Non_homology/all_human_homology_regions.bed | wc -l

	#Save Intergenic SCFRs with no overlap with homolohy regions
	bedtools intersect -a human_scfr_all_atleast_300bp_only_intergenic_unique.bed -b Non_homology/all_human_homology_regions.bed -v > human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology.bed
	bedtools intersect -a human_scfr_all_atleast_300bp_antisense_intergenic.bed -b Non_homology/all_human_homology_regions.bed -v > human_scfr_all_atleast_300bp_antisense_intergenic_with_no_homology.bed
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#2. Concatenate DFT results with pure intergenic SCFRs with no homology
	...
	#scp human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_dft_results.bed ceglab8@172.28.65.118:~/Downloads/SCFR/

	#View in IGV (inceglab8)
	/home/ceglab8/workspace/1.master_thesis/6.source_codes/IGV_Linux_2.5.0/igv.sh
	#or
	/home/ceglab8/miniforge3/share/igvtools-2.17.3-0/igv.sh

#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#3. Filter only SCFRs with 0.3 / 0.33 DFT frequency
	...
	awk -F "\t" 'BEGIN{OFS="\t"} {print ($3 - $2), $0}' human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_0.33_dft_results.bed | sort -k1,1nr | cut -f2- > length_sorted_human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_0.33_dft_results.bed

#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#Visualize

#Explorartory visualization of the intergenic SCFR with 0.33 DFT frequency fogenomic / chromosome patterns:

	mkdir /media/aswin/SCFR/SCFR-main/Fourier_analysis/human/300/visualize
	cd /media/aswin/SCFR/SCFR-main/Fourier_analysis/human/300/visualize
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#Add header
sed -i '1i Chr Start End Frame_strand Filler Strand SCFR_Name Num_Raw_Peaks Top_Peak_Count Frequencies Magnitudes Periods' length_sorted_human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_0.33_dft_results.bed 
#make tab separated
sed -i 's/[ ]\+/\t/g' length_sorted_human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_0.33_dft_results.bed
#Make it standard bed format: Take 1st 6 columns
awk -F "\t" '{print$1,$2,$3,$4,$5,$6}' OFS="\t" human_scfr_all_atleast_300bp_only_intergenic_unique_with_no_homology_with_0.33_dft_results.bed > ucsc.bed
...
######################################################################################################################################################################################################################################################################################################
#4. Download NCBI Refseq Functional elements for human T2T genome from UCSC
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#5. Download RepeatMasker data for human from UCSC
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#6. Download RNA-seq track data from UCSC

wget https://hgdownload.soe.ucsc.edu/gbdb/hs1/bbi/xenoRefGene.bb

######################################################################################################################################################################################################################################################################################################
#7. Remove SCFRs partially overlapping genes
	...
	#NOTE: SCFRs overlapping genes (whole genic region) are also overlapping with all SCFRs overlapping other features like UTR, CDS, introns, RNA-genes, etc, as these are essentially gene

	#Remove SCFRs woth partial overlaps
...
######################################################################################################################################################################################################################################################################################################
#8. Remove SCFRs with homology to any existing proteins/genes

#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#8.1. Prepare input fasta
...
######################################################################################################################################################################################################################################################################################################
#8.2. Remove SCFRs with homology to any existing proteins
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#8.2.1.1. Export NR blastx output
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#8.2.1.2. Remove SCFRs overlapping nr blastx hits
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#Collect all intergenic SCFRs at all filtering stages
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#8.2.1.3. Check scfrs overlapping repeats
...
#Prepare inputs
	mkdir -p ~/aswin/SCFR/Fourier_analysis/nt_blast
